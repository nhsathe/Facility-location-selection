...
model.object= Objective(expr = sum(sum(0.1*model.p[j]*model.x[i,j]*pyo.value(model.d[i,j]) for i in model.I) for j in model.J),sense=minimize)
...
st.write("Support center should be built at zipcodes:",Zipcode1[i-1])
model.pprint()
