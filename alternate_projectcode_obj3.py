# -*- coding: utf-8 -*-
"""
Created on Sat Dec  3 22:33:53 2022
...
#df.to_excel(excel_writer = "D:/MSIE sem 1/IE 8030 Engineering optimaiztion and applications/Project/distance.xlsx")
...
model.object= Objective(expr = sum(sum(0.1*model.p[j]*model.x[i,j]*model.s[i,j] for i in model.I) for j in model.J),sense=maximize)
    
                        
    

def distance_rule(model,i,j):
    if(pyo.value(model.d[i,j])<=5):
